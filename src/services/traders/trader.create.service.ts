...
export const create = (usersRepository: UsersRepository, traderRepository: TraderRepository) =>
	async ({ address, cpf, storeName, storeType, userId, cnpj, cep, linkMap, createdAt, updatedAt }: TradersDataDTO) => {
		try {
			...
			const traderEntity = new TraderEntity({ address, cpf, storeName, storeType, userId, cep, cnpj, linkMap, createdAt, updatedAt });
